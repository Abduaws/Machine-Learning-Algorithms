...
def loadDataFile(filename, n, width, height):
    ...
    for i in range(n):
        ...
        if len(data[0]) < width - 1:
            ...
            print(f"Truncating at %d examples {i}")
            break
        items.append(Datum(data, width, height).getPixels())
    return items
...
def convertToInteger(data):
    ...
    for i in range(len(data)):
        data[i] = list(map(IntegerConversionFunction, data[i]))
    return data
