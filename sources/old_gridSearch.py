...
class GraphDia(QtWidgets.QDialog):
    def __init__(self, pairs, xlabel="Sample Sizes", graph_type="line"):
        super().__init__()
...
class MLP_Tune_Dia(QtWidgets.QDialog):
    ...
    def workerCode(self):
        ...
        if self.activ_check.isChecked():
            grid_params['activation'] = ["identity", "tanh", "relu"]
        if self.checkBox_2.isChecked():
            grid_params['learning_rate_init'] = [0.001, 0.01, 0.1]
        if self.checkBox.isChecked():
            grid_params['max_iter'] = [200, 250, 300, 350, 400]
        X_test, Y_test, X, Y = self.load_data()
        mlp = MLPClassifier()
        mlp.fit(X, Y)
        Y_pred = mlp.predict(X_test)
        mlpTuned = HalvingGridSearchCV(MLPClassifier(), grid_params, refit=True, cv=3, n_jobs=-1)
        mlpTuned.fit(X, Y)
        Y_predTuned = mlpTuned.predict(X_test)
        self.accBefore = round(accuracy_score(Y_test, Y_pred) * 100, 5)
        self.accAfter = round(accuracy_score(Y_test, Y_predTuned) * 100, 5)
        self.temp_output = f"Best Parameters:\n"
        if self.activ_check.isChecked():
            self.temp_output += f"   Activation Function: {mlpTuned.best_params_['activation']}\n"
            self.actFunc = mlpTuned.best_params_['activation']
        if self.checkBox_2.isChecked():
            self.temp_output += f"   Learning Rate: {mlpTuned.best_params_['learning_rate_init']}\n"
            self.learningRate = mlpTuned.best_params_['learning_rate_init']
        if self.checkBox.isChecked():
            self.temp_output += f"   No Of Epoch: {mlpTuned.best_params_['max_iter']}\n\n"
            self.noEpoch = mlpTuned.best_params_['max_iter']
...
class SVM_Tune_Dia(QtWidgets.QDialog):
    ...
    def workerCode(self):
        ...
        if self.gamma_check.isChecked():
            grid_params['gamma'] = [1, 0.1, 0.01, 0.001, 'scale', 'auto']
        if self.c_check.isChecked():
            grid_params['C'] = [0.1, 1, 10, 100]
        svm = SVC()
        svm.fit(X, Y)
        Y_pred = svm.predict(X_test)
        svmTuned = HalvingGridSearchCV(SVC(), grid_params, refit=True, cv=3, n_jobs=-1)
        svmTuned.fit(X, Y)
        Y_predTuned = svmTuned.predict(X_test)
        self.accBefore = round(accuracy_score(Y_test, Y_pred) * 100, 5)
        self.accAfter = round(accuracy_score(Y_test, Y_predTuned) * 100, 5)
        self.temp_output = f"Best Parameters:\n"
        if self.c_check.isChecked():
            self.temp_output += f"   C: {svmTuned.best_params_['C']}\n"
            self.c = svmTuned.best_params_['C']
        if self.gamma_check.isChecked():
            self.temp_output += f"   Gamma: {svmTuned.best_params_['gamma']}\n\n"
            self.gamma = svmTuned.best_params_['gamma']
        self.temp_output += f"Accuracy Comparison:\n" \
                            f"   Accuracy Before Tuning: {self.accBefore}%\n   Accuracy After Tuning: {self.accAfter}%"
        self.done = True
...
class Decision_Tune_Dia(QtWidgets.QDialog):
    ...
    def workerCode(self):
        ...
        decisionTuned = HalvingGridSearchCV(DecisionTreeClassifier(), self.grid_params, refit=True, cv=5, n_jobs=-1)
        decisionTuned.fit(X, Y)
        Y_predTuned = decisionTuned.predict(X_test)
        self.accBefore = round(accuracy_score(Y_test, Y_pred) * 100, 5)
        self.accAfter = round(accuracy_score(Y_test, Y_predTuned) * 100, 5)
        self.temp_output = f"Best Parameters:\n"
        if self.crit_check.isChecked() or self.mode == 'auto':
            self.temp_output += f"   Criterion: {decisionTuned.best_params_['criterion']}\n"
            self.criterion = decisionTuned.best_params_['criterion']
        self.temp_output += f"   Max Depth: {decisionTuned.best_params_['max_depth']}\n"
        self.maxDepth = decisionTuned.best_params_['max_depth']
        self.temp_output += f"   Max Leaves: {decisionTuned.best_params_['max_leaf_nodes']}\n\n"
        self.maxLeafs = decisionTuned.best_params_['max_leaf_nodes']
        self.temp_output += f"Accuracy Comparison:\n" \
                            f"   Accuracy Before Tuning: {self.accBefore}%\n   Accuracy After Tuning: {self.accAfter}%"
        self.done = True
    ...
    def go_Press(self, mode):
        ...
        try:
            if self.mode == "manual":
                ...
                self.grid_params = {'max_depth': [i for i in range(int(self.start_depth.text()), int(self.end_depth.text()), 20)],
...
class Ui_MainWindow(object):
    ...
    def Decision_Train_Test(self, train_sizes, test_size):
        ...
        for sample_size in train_sizes:
            X, Y = self.load_train_data(sample_size)
            des = DecisionTreeClassifier(criterion=self.criterion, max_depth=self.maxDepth, max_leaf_nodes=self.maxLeafs)
            des.fit(X, Y)
    ...
    def setup_Press(self):
        ...
        if self.classifiercombo.currentText() == "Naive Bayes":
            ...
            naiveDia.exec_()
            if naiveDia.getOut: self.vsmooth = naiveDia.vsmooth
            else:self.vsmooth = 1e-9
        elif self.classifiercombo.currentText() == "KNN":
            knnDia = KNN_Tune_Dia(sample)
            knnDia.exec_()
            if knnDia.getOut: self.knnK = knnDia.k
            else:self.knnK = 2
